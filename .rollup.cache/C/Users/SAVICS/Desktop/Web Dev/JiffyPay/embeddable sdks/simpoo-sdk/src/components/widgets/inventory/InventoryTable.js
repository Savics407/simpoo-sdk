...
export const InventoryTable = () => {
    ...
    const tableData = itemsData?.data?.map((item) => {
        const getFirstImage = () => {
    ...
    const { data: counts, isFetching: loadingCounts } = useFetchData(["items-tab"], `/outbound/items/count`, {
        enabled: !!apiKey,
    });
    ...
    return (_jsxs(Container, { className: "px-4 py-6 flex flex-col gap-8", children: [_jsx(Tabs, { tabs: tabsData, loading: loadingItems }), _jsxs("div", { className: " flex items-center justify-between flex-wrap gap-4", children: [_jsxs("div", { className: "flex items-center gap-2.5", children: [_jsx(SearchComponent, { placeholder: "Search Item name, barcode or sku", value: searchValue, onChange: (e) => setSearchValue(e.target.value) }), loadingItems ? (_jsx(Skeleton, { className: "w-20 h-6" })) : (_jsxs("p", { className: "text-dark text-sm font-medium", children: [itemsData?.meta?.total ?? 0, " ", _jsx("span", { className: "text-gray-700", children: "Items" })] }))] }), _jsx("div", { className: "flex gap-4", children: _jsx(Button, { label: "Create New", left_addon: icons.plus_squared, className: "!font-medium !text-sm !h-12", onClick: () => setMultipleItems(true) }) })] }), _jsx(ItemsOverview, { loading: loadingItems, data: items?.data?.[1], meta: itemsData?.meta }), _jsx(Table, { loading: loadingItems, columns: itemColumns, data: tableData, emptyData: tableData.length === 0, emptyProps: {
                    header: "No item yet",
                    subText: "Add new item to start managing your inventory.",
                }, tHeadClass: "whitespace-nowrap", tbCellClass: "text-dark font-semibold", meta: itemsData?.meta }), multipleItems ? (_jsx(CreateMuiltipleItems, { isOpen: multipleItems, onClose: () => setMultipleItems(false) })) : null] }));
};
